...
class TuioCursorImage(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.brushing_sound_source = os.path.join(script_path, 'brushing.mp3')
...
class MenuModalView(ModalView):
    ...
    def __init__(self, game_screen, **kwargs):
        ...
        self.background_color = (0, 0, 0, 0.85)  # Semi-transparent dark background
        self.background = os.path.join(self.script_path, "assets/menubackground.png")  # Remove default background
        ...
        play_button = Button(
            ...
            background_normal = os.path.join(self.script_path, "assets/start_icon.png"),
        )
        ...
        ranking_button = Button(
            ...
            background_normal = os.path.join(self.script_path, "assets/ranking_icon.png"),
        )
    ...
    def show_ranking(self, instance):
        """Opens ranking screen (to be implemented)"""
        print("Opening ranking screen (not yet implemented)")
        # TODO: Implement RankingModalView and open it here
        # ranking_view = RankingModalView()
        # ranking_view.open()
...
class GameScreen(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Background music
        bg_path = os.path.join(self.script_path, 'sounds/background.mp3')
        ...
        # Font
        font_path = os.path.join(self.script_path, '')
    ...
    def setup_game(self):
        ...
        json_path = os.path.join(self.script_path, 'config/teethconfig.json') 
        ...
        # Clean sound
        clean_sound_path = os.path.join(self.script_path, 'sounds/clean.mp3')
        self.clean_sound = SoundLoader.load(clean_sound_path)

        # Background image
        self.add_widget(Image(
            source=os.path.join(self.script_path, 'assets/Fondo.png'),
            allow_stretch=True,
            keep_ratio=False
        ))
        ...
        for tooth_id in sorted(teethConfig.keys()):
            ...
            if not is_clean:
                ...
                self.filthVisuals[tooth_id] = filth_layer
                self.add_widget(filth_layer)

                # Foam visual
                foam_visual = Image(
                    source=os.path.join(self.script_path, 'assets/espuma.gif'),
                ...
                self.foamVisuals[tooth_id] = foam_visual
                self.add_widget(foam_visual)

                # Layer cleaned animation
                clean_gif = Image(
                    source=os.path.join(self.script_path, 'assets/layercleaned.gif'),
                    size_hint=(0.3, 0.3), 
                    keep_ratio=False,
                    anim_delay=-1,
                    opacity=0,
                ...
                self.sparkleVisuals[tooth_id] = clean_gif
            ...
            self.teethLogicList[tooth_id] = tooth_logic
            self.add_widget(tooth_logic)

        # Skin overlay
        self.add_widget(Image(
            source=os.path.join(self.script_path, 'assets/Piel (caraserhumano).png'),
            allow_stretch=True,
            keep_ratio=False
        ))
    ...
    def handle_brush_stroke(self, tooth_id):
        ...
        if tooth_id not in self.teethLogicList:
            print(f"Tooth {tooth_id} not found")
            return
            
        tooth_logic = self.teethLogicList[tooth_id]
        filth_visual = self.filthVisuals[tooth_id]
        sparkle_visual = self.sparkleVisuals[tooth_id]
    ...
    def check_for_win(self):
        ...
        for tooth in self.teethLogicList.values():
    ...
    def start_foam_animation(self, tooth_id):
        ...
        if tooth_id in self.foamVisuals:
            foam_visual = self.foamVisuals[tooth_id]
